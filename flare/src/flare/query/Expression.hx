package flare.query;
	 ...
	 */
	class Expression implements IEvaluable, IPredicate
	{	
		 ...
		 */
		public function eval(?o:Dynamic=null):Dynamic
		{
			return o;
		}
		 ...
		 */
		public function predicate(o:Dynamic):Bool
		{
		 ...
		 *  false otherwise
		 */
		public function setChildAt(idx:Int, expr:Expression):Bool
		{
			return false;
		}
		 ...
		 */
		public function visit(f:Dynamic):Bool
		{
			var iter:ExpressionIterator = new ExpressionIterator(this);
			return visitHelper(iter, f);
		}
		
		private function visitHelper(iter:ExpressionIterator, f:Dynamic):Bool
		{
			if (cast( f(iter.current), Boolean)) return true;
			if (iter.down() != null)
			{
				do {
		...
		inline private static var _SQUOTE:Int = "'".charCodeAt(0);
		inline private static var _DQUOTE:Int = "\"".charCodeAt(0);
		 ...
		 */
		public static function expr(o:Dynamic):Expression
		{
			if (Std.is( o, Expression)) {
				return cast( o, Expression);
			} else if (Std.is( o, Class)) {
				return new IsA(Class(o));
			} else if (Std.is( o, String)) {
				var s:String = cast( o, String);
				var c1:Int = s.charCodeAt(0);
				var c2:Int = s.charCodeAt(s.length-1);
