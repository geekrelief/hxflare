package flare.util;
	...
	{		
         ...
         */
        public static function bsplineCoeff(p:Array<Dynamic>, a:Array<Dynamic>, b:Array<Dynamic>):Void
        {
         ...
         */
        public static function bspline(u:Number, a:Array<Dynamic>, b:Array<Dynamic>, s:Point) : Point
        {
    	...
    	inline public static var NO_INTERSECTION:Int = 0;
    	/** Indicates intersection between shapes */
    	inline public static var COINCIDENT:Int      = -1;
    	/** Indicates two lines are parallel */
    	inline public static var PARALLEL:Int        = -2;
	    ...
	    {
	        var ua_t:Int = (b2x-b1x)*(a1y-b1y)-(b2y-b1y)*(a1x-b1x);
	        var ub_t:Int = (a2x-a1x)*(a1y-b1y)-(a2y-a1y)*(a1x-b1x);
	        var u_b:Int = (b2y-b1y)*(a2x-a1x)-(b2x-b1x)*(a2y-a1y);
	    ...
	    {
	        var xMax:Int = r.right, yMax:Int = r.bottom;
	        var xMin:Int = r.left,  yMin:Int = r.top;
         ...
         */
        public static function convexHull(p:Array<Dynamic>, len:UInt) : Array<Dynamic>
        {
        	// check arguments
            if (len < 3)
            	throw new ArgumentError('Input must have at least 3 points');
            
            var pts:Array<Dynamic> = new Array(len-1);
            var stack:Array<Dynamic> = new Array(len);
            ...
            for (i in 1...len-1) {
                ...
                if (angle == pts[i].angle)
                {
                    ...
                    var d1:Int = Point.distance(p[i0], p[tj]);
                    var d2:Int = Point.distance(p[i0], p[j]);
            ...
            // construct the hull
            var hull:Array<Dynamic> = new Array(sp);
            for (i in 0...sp) {
                hull[i] = p[stack[i]];
            }
            return hull;
        ...
        private static function isNonLeft(i0:UInt, i1:UInt, i2:UInt, i3:UInt, p:Array<Dynamic>) : Bool
        {
