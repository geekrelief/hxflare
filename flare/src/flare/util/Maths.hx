package flare.util;
	...
	class Maths
	{
		inline private static var EPSILON:Float = 1e-30;
         ...
         */
        public static function quantile(n:UInt, values:Array<Dynamic>, sort:Bool):Array<Dynamic>
        {
        	var len:UInt = values.length-1;
			var qtls:Array<Dynamic> = new Array(n);
		 ...
		 * @return the interpolated value
		 */
		public static function quantileInterp(f:Number, quantiles:Array<Dynamic>):Number
		{
        ...
        {
            min = symLog(min, b);
            var denom:Int = symLog(max, b) - min;
        ...
        {
            min = adjLog(min, b);
            var denom:Int = adjLog(max, b) - min;
        ...
        {
            min = symRoot(min,p);
            var denom:Int = symRoot(max,p) - min;
		 ...
		 * @return the inferred interpolation fraction
		 */
		public static function invQuantileInterp(x:Number, quantiles:/*Number*/Array<Dynamic>):Number
        {
         ...
         */
        public static function noise(x:Number, ?y:Int=0, ?z:Int=0):Number
   		{
		...
		private static function grad(h:Int, x:Number, y:Number, z:Number):Number
		{
			h &= 15;
			var u:Int = h<8 ? x : y;
			var v:Int = h<4 ? y : h==2||h==14 ? x : z;
			return ((h&1) == 0 ? u : -u) + ((h&2) == 0 ? v : -v);
		}
		
		inline private static var _p:Array<Dynamic> = [151,160,137,91,90,15,131,13,201,95,96,
