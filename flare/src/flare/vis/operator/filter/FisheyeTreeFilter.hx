package flare.vis.operator.filter;
	 ...
	 */
	class FisheyeTreeFilter extends Operator
	{
		 ...
		 */		
		public function new(?focusNodes:Array<Dynamic>=null,?distance:Int=1)
		{
		 ...
		 */
		private function setVisibility(ds:DataSprite, visible:Bool):Void
		{
			var alpha:Int = visible ? 1 : 0;
			var obj:Dynamic = _t._S_(ds);
				
			obj.alpha = alpha;
			if (Std.is( ds, NodeSprite))
				(cast( ds, NodeSprite)).expanded = (ds.props.doi > -distance);
			if (_t.immediate) {
				ds.visible = visible;
			} else {
				obj.visible = visible;
	    ...
	    private function visit(n:NodeSprite, c:NodeSprite, doi:Int, ldist:Int):Void
	    {
	        ...
	        var localDOI:Float = -ldist / Math.min(1000.0, _divisor);
