package flare.vis.operator.layout;
	 ...
	 */
	class DendrogramLayout extends Layout
	{
		public var distanceProperty(getDistanceProperty, setDistanceProperty) : String;
		public var orientation(getOrientation, setOrientation) : String;
		// TODO: support axes, too
		
		private var _orient:String ; // the orientation of the tree
		private var _dp:Property;
		
		private var _leafCount:Int;
		private var _leafIndex:Int ;
		...
		public function getDistanceProperty():String { return _dp.name; }
		public function setDistanceProperty(dp:String):String {
			_dp = Property._S_(dp);
			return dp;
		}
		 ...
		 */
		public function new(?distField:String=null,
			?orientation:String=Orientation.TOP_TO_BOTTOM)
		{
			
			_orient = Orientation.TOP_TO_BOTTOM;
			_leafIndex = 0;
			_dp = distField==null ? null : Property._S_(distField);
			_orient = orientation;
		}
		...
		{
			var d:Int = _dp!=null ? _dp.getValue(n) : n.u;
			d = _d1 + _dd * (d / _maxDist);
			
			if (n.childDegree > 0) {
				var b:Int = 0, bc:Number;
				for (i in 0...n.childDegree) {
