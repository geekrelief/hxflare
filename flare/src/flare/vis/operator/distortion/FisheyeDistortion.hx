package flare.vis.operator.distortion;
	 ...
	 * </p>
	 */
	class FisheyeDistortion extends Distortion
	{
	     ...
	     */
		public function new(?dx:Int=4, ?dy:Int=4, ?ds:Int=3) {
			_dx = dx;
			_dy = dy;
			_ds = ds;
		...
		/** @inheritDoc */
		public override function xDistort(x:Number):Number
		{
			return fisheye(x, layoutAnchor.x, _dx, _b.left, _b.right);
		}
		
		/** @inheritDoc */
		public override function yDistort(y:Number):Number
		{
			return fisheye(y, layoutAnchor.y, _dy, _b.top, _b.bottom);
		}
		
		/** @inheritDoc */
		public override function sizeDistort(bb:Rectangle, x:Number, y:Number):Number
		{
			if (!distortX && !distortY) return 1;
			var fx:Int=1, fy:Int=1;
	        ...
	        var sf:Int = (!distortY ? fx : (!distortX ? fy : Math.min(fx,fy)));
			...
			min:Number, max:Number) : Number
		{
	        ...
	        var left:Bool = x<a;
	        var v:Number, m:Int = (left ? a-min : max-a);
	        if ( m == 0 ) m = max-min;
	        v = Math.abs(x - a) / m;
	        v = (d+1)/(d+(1/v));
